...
export function MenuManager({ categories, items, mode = "catalog" }: { categories: { id: string; name: string }[]; items: MenuItem[]; mode?: "catalog" | "new" }) {
  ...
  return <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
    <PageHeading eyebrow="Restaurant administration" title="Menu studio" description="Build the single menu your cashiers sell and guests see on the customer display." actions={<Button href="/admin/menu/new" variant="secondary" size="sm">+ New menu item</Button>} />
    ...
    {mode === "catalog" ? <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric icon="≡" label="Menu items" value={String(itemCount)} detail="Shared with POS & display" /><Metric icon="□" label="Categories" value={String(categories.length)} detail="Organise the guest menu" /><Metric icon="±" label="Modifier groups" value={String(modifierCount)} detail="Options for made-to-order items" /><Metric icon="◷" label="Sold out" value={String(unavailable)} detail="Hidden from sales" /></section> : null}
    <section className={`mt-6 grid gap-5 ${mode === "new" ? "xl:grid-cols-[minmax(0,1fr)_390px]" : "xl:grid-cols-1"}`}>
