...
export default async function NewMenuItemPage() {
  ...
  return <MenuManager mode="new" categories={categories.map((category) => ({ id: String(category._id), name: category.name }))} items={products.map((product) => ({
    id: String(product._id), name: product.name, description: product.description ?? "", categoryId: String(product.categoryId), category: categoryNames.get(String(product.categoryId)) ?? "Uncategorised", kitchenStation: product.kitchenStation ?? "", sku: product.sku ?? "", barcode: product.barcode ?? "", taxRatePercent: product.taxRatePercent ?? undefined, isAvailable: product.isAvailable ?? true, imageUrl: cloudinaryImageUrl(product.imagePublicId),
