...
export function PosClient({
  ...
  return (
    <main className="min-h-screen bg-paper text-ink dark:bg-ink dark:text-paper print:bg-white print:text-black">
      ...
      {activeTicket ? <><button type="button" aria-label="Close payment panel" onClick={() => setActiveTicket(null)} className="pos-payment-backdrop fixed inset-0 z-40 cursor-default bg-ink/55 backdrop-blur-sm print:hidden" /><button type="button" onClick={() => setActiveTicket(null)} className="fixed right-5 top-5 z-[60] inline-flex items-center gap-2 rounded-full border border-ink-line/20 bg-white px-3 py-2 text-xs font-semibold text-ink shadow-lg hover:bg-paper-dim dark:border-ink-line dark:bg-ink-soft dark:text-paper dark:hover:bg-ink-line print:hidden"><span aria-hidden>×</span> Close</button></> : null}
