...
export default async function PosPage() {
  ...
  const [products, organization, categories, tables, customers, openTickets] = await Promise.all([
    ...
    ctx.activeBranchId
      ? OrderModel.find({
          ...
          // Keep recently completed tickets available here as well: a manager
          // must be able to reconcile or refund a completed payment from POS.
          status: { $in: ["DRAFT", "PLACED", "CONFIRMED", "PREPARING", "READY", "COMPLETED"] },
        })
          .sort({ updatedAt: -1 })
          .limit(30)
          .lean()
  ...
  const menu = products.map((product) => ({
    ...
    modifierGroups: product.modifierGroups.map((group) => ({
      ...
      options: group.options.map((option) => ({ id: String(option._id), name: option.name, priceMinor: option.priceMinor })),
    })),
  }));
  ...
  const paymentMethods = configuredPaymentMethods.length ? configuredPaymentMethods : [
    ...
    { code: "BANK" as const, label: "Bank transfer" },
  ...
  return (
    <PosClient
      ...
      openTickets={openTickets.map((order) => ({
        id: String(order._id),
