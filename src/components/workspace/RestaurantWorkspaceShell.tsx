  ...
  return (
    <div className="min-h-screen bg-paper text-ink dark:bg-ink dark:text-paper">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-ink-line bg-ink text-paper transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "lg:-translate-x-full" : "lg:translate-x-0"}`}>
        ...
        <div className="border-t border-ink-line p-3"><div className="mb-2 flex items-center justify-between rounded-lg bg-paper/8 px-3 py-2"><span className="text-xs text-paper/60">Plan</span><span className="text-xs font-medium text-indigo-200">{plan}</span></div><button type="button" onClick={() => setCollapsed(true)} className="hidden w-full rounded-lg px-3 py-2 text-left text-xs text-paper/50 transition-colors hover:bg-paper/10 hover:text-paper lg:block">← Collapse sidebar</button><form action={logoutAction}><button className="w-full rounded-lg px-3 py-2 text-left text-xs text-paper/60 transition-colors hover:bg-paper/10 hover:text-paper">Sign out</button></form></div>
      </aside>
