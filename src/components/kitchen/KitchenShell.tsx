...
export function KitchenShell({ children, branchName, stations }: { children: React.ReactNode; branchName: string; stations: string[] }) {
  ...
  return <div className="kitchen-surface min-h-screen bg-paper text-ink dark:bg-ink dark:text-paper">
    <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-ink-line/15 bg-paper/95 px-4 py-2 backdrop-blur dark:border-ink-line dark:bg-ink/95 sm:gap-4 sm:px-6">
      <Link href="/workspace" className="flex min-w-0 items-center gap-2 border-r border-ink-line/15 pr-3 dark:border-ink-line sm:pr-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-amber-400 text-lg text-ink">♨</span><span className="min-w-0"><b className="block truncate text-sm">Kitchen OS</b><small className="block truncate text-[9px] uppercase tracking-[.16em] text-ink/45 dark:text-paper/45">Restaurant operations</small></span></Link>
    ...
    <div className="min-h-[calc(100vh-4rem)] lg:pl-56">
      <aside className="hidden w-56 shrink-0 border-r border-ink-line/15 bg-white p-3 dark:border-ink-line dark:bg-ink-soft lg:fixed lg:inset-y-16 lg:left-0 lg:z-10 lg:flex lg:flex-col">
        ...
        <div className="mt-auto space-y-2"><div className="rounded-lg border border-emerald-400/25 bg-emerald-400/10 p-3 text-xs text-emerald-700 dark:text-emerald-200"><span className="mr-2 text-emerald-500 dark:text-emerald-400">●</span>Kitchen display connected</div><Link href="/workspace" className="flex items-center justify-center rounded-lg border border-ink-line/20 px-3 py-2.5 text-sm font-medium text-ink/70 transition-colors hover:bg-paper-dim hover:text-ink dark:border-ink-line dark:text-paper/70 dark:hover:bg-paper/10 dark:hover:text-paper">← Back to workspace</Link></div>
      </aside>
